...
The Three.js 3D scenes are pre-built and checked into `assets/three-scenes.js`. If you modify `src/three/*.ts`, rebuild:
...
The built file (`assets/three-scenes.js`) should be committed to the repository since Render doesn't run the Node build step.
...
| Model file not found | `frozen models/` missing from deploy | Ensure the directory and `.pkl` file are committed to Git (not in `.gitignore`) |
