...
| `assets/` | Static files | Auto-served by Dash. CSS, JS, logos. |
| `src/three/` | Three.js TypeScript | Compiled to `assets/three-scenes.js` via esbuild. Run `npm run build` after changes. |
| `models/` | ML artifacts | Read-only. Do not modify without retraining. |
...
- The `.gitignore` excludes `.env`, `__pycache__/`, `.venv/`, `.DS_Store`, `.idea/`, `node_modules/`, `.cache/`, and `*.log`.
